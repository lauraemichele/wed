...
- **Single "Aggiungi al calendario" button** (`#add-to-calendar`) — `generateCalendarLinks()` in `js/main.js` upgrades the href at runtime:
  - **Android Chrome** (UA-sniffed; excludes Edge/Samsung Internet/Firefox/Opera): swapped to an `intent://` URL with `action=android.intent.action.INSERT` and string/long extras. Bypasses the .ics download — directly opens the system "Nuovo evento" UI with title/location/time pre-filled, and the user picks which calendar app saves it.
