...
- **Single "Aggiungi al calendario" button** (`#add-to-calendar`) — `generateCalendarLinks()` in `js/main.js` upgrades the href at runtime:
  - **Android Chrome** (UA-sniffed; excludes Edge/Samsung Internet/Firefox/Opera): swapped to a type-based intent targeting the Google Calendar package (`intent:#Intent;package=com.google.android.calendar;action=android.intent.action.INSERT;category=android.intent.category.DEFAULT;type=vnd.android.cursor.item/event;...;end`) — this combo matches GCal's manifest intent-filter exactly. `target="_blank"` is removed since it breaks intent dispatch on Chrome. `browser_fallback_url` points to the static `.ics` — so if Google Calendar isn't installed the user gets the file instead of being bounced to the web Google Calendar.
